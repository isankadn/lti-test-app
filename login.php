 ...
 * This endpoint handles OIDC authentication responses from Saltire tool
...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    // This endpoint receives the OIDC response from Saltire
    // Saltire will redirect here with authentication parameters

    logMessage("OIDC response from Saltire received", $params, 'LOGIN');
    ...
    // Validate issuer (should be Saltire)
    if ($params['iss'] !== SALTIRE_TOOL_DOMAIN) {
        logMessage("Invalid issuer", ['received' => $params['iss'], 'expected' => SALTIRE_TOOL_DOMAIN], 'LOGIN');
        http_response_code(400);
    ...
    $authParams = [
        ...
        'redirect_uri' => $params['target_link_uri'] ?? SALTIRE_LAUNCH_URL,
